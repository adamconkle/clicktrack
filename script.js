...
function flashScreen(accent) {
  flash.classList.remove("flash-accent", "flash-regular");
  void flash.offsetWidth; // reset animation
  flash.classList.add(accent ? "flash-accent" : "flash-regular");
  setTimeout(() => flash.classList.remove("flash-accent", "flash-regular"), 150);
}
...
function startClickTrack() {
  ...
  intervalId = setInterval(() => {
    ...
    const currentBeat = Math.floor(beatCounter % beatsPerMeasure);
    
    playClick(isAccent);
    drawVisuals(beatsPerMeasure, currentBeat, isAccent);
    flashScreen(isAccent);
